# Hi, I'm Hamza Akram 👋

🚀 Full-Stack Developer (Agentic AI • React • Node.js • ExpressJS • AWS) | Building scalable web apps & modern dashboards

---

## 👨‍💻 About Me

- 🔥 5+ years of experience in Frontend Development (React.js, JavaScript)
- 🧠 Currently transitioning into Full-Stack development (Node.js + NestJS)
- 🏨 Working on a Hotel Inventory & Booking Management System
- 🤖 Interested in Agentic AI, AI Interfaces, real-time avatars, and low-latency systems

---

## 🛠️ Tech Stack

### Frontend
- React.js, Next.js, JavaScript, TypeScript
- HTML, CSS, Tailwind, Bootstrap

### Backend
- Node.js, Express.js, NestJS
- REST APIs, Authentication, Role-based access

### Database
- MySQL, PostgreSQL, MongoDB

### Tools & DevOps
- Git, GitHub, Postman
- AWS, GCP, Cloud Run
- Google Analytics

---

## 📌 Featured Projects

### 🏨 Hotel Management System
- Inventory per hotel
- Booking system + key card assignment
- Admin web portal + staff mobile app

### 🦷 Dental Clinic Management
- Appointment scheduling for single doctor
- Working hours (10 AM - 10 PM)
- Staff workflow optimization

### 🎭 AI Real-Time Avatar (Urdu Support)
- Exploring WebRTC + OpenAI real-time models
- Focused on reducing latency for Urdu (Pakistan)

---

## 📫 Contact Me

- 📧 Email: hkakram97@gmail.com  
- 💼 LinkedIn: [https://linkedin.com/in/hamza-akram](https://www.linkedin.com/in/muhammad-hamza-akram-akram/)  
- 🌐 Portfolio: (Coming soon :)  

---

⭐ If you like my work, feel free to follow and connect!
